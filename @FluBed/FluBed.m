...
%                 phi_s=phi(eps_mf);
        ...
        function w_t=w_t(d_p,rho_p,phi_s,p,T)
            %Terminal velocity, Kunii p. 80
            d_pAst=FluBed.Ar(d_p,rho_p,p,T).^(1/3);
            w_tAst=(18./d_pAst.^2+(2.335-1.744.*phi_s)./sqrt(d_pAst)).^-1;
            
            rho_g=DryAir.rho(p,T);
            w_t=w_tAst.*(rho_g.^2./(DryAir.eta(T).*(rho_p-rho_g).*FluBed.g)).^(-1/3);
        end
...
%             d_bm=0.65/100*(A*10^4.*(w0-wmf)*100).^0.4;   %Maximum bubble diameter, K/L p.146
        ...
        function Gamma=Gamma(w,href,d_p,rho_p,p,T)
            ...
            sz=implExp.size(w,href,d_p,rho_p,p,T);
            [w,href,d_p,rho_p,p,T]=implExp.normalize(sz,w,href,d_p,rho_p,p,T);
        ...
        function [h,Nu]=molerus(w,T_b,T_s,p,d_p,rho_p,eps_mf)
            %Heat transfer coefficient according to Molerus
        ...
        function d_pMin=elutriation(d_p,rho_p,phi_s,eps_mf,p,T,FG)
            %Implicit expansion
            sz=implExp.size(d_p,rho_p,phi_s,eps_mf,p,T,FG);
            [d_p,rho_p,phi_s,eps_mf,p,T,FG]=implExp.normalize(sz,d_p,rho_p,phi_s,eps_mf,p,T,FG);
            
            
            wmf=FluBed.wmfErgun(d_p,rho_p,phi_s,eps_mf,p,T);
            w0=FG.*wmf;            
            
            d_pMin=arrayfun(@(i) fzero(@(d_p) FluBed.w_t(d_p,rho_p(i),phi_s(i),p(i),T(i))-w0(i),[0,max(d_p,[],'all')]),1:numel(w0));
            d_pMin=reshape(d_pMin,sz);
        end


        function [FGdiff,FG,p1,p2,mDotS]=FGdist(d_p,rho_p,eps,FG_ref,p0,name,s,Tbed,h)
            if ischar(name)
                name={name};
            end

            reg={d_p,rho_p,eps,FG_ref,p0,name,s};
            disc={Tbed,h};
            sz=implExp.sizeDisc(reg,disc,1);
            [reg,disc]=implExp.normDisc(sz,reg,disc,1);

            reg=cell2struct(reg,{'d_p','rho_p','eps','FG_ref','p0','name','s'},2);
            disc=cell2struct(disc,{'T','h'},2);

            center=round(mean([1,size(disc.T,1)]));

            p=reg.p0+FluBed.deltaP(disc.h./2,reg.eps,reg.rho_p);
            p2=reg.p0+FluBed.deltaP(disc.h,reg.eps,reg.rho_p);

            
            %Reference point (ref): bed center
            p_ref=p(center,:);
            T_ref=disc.T(center,:);
            wmf_ref=FluBed.wmf(reg.d_p,reg.rho_p,p_ref,T_ref);
            w_ref=reg.FG_ref.*wmf_ref;
            ...
            p1=arrayfun(@(i) ...
                    fzero(@(p1) ...
                        Sinter.w(p1,p2_ref(i),T_ref(i),reg.s(i),reg.name(i))-w2_ref(i),...
                        [p2_ref(i),p2_ref(i).*100]),...
                    1:length(p2_ref));
            [~,~,mDotS]=Sinter.w(p1,p2,disc.T,reg.s,reg.name);
            
            
            rho=DryAir.rho(p,disc.T);
            w=mDotS./rho;
            wmf=FluBed.wmf(reg.d_p,reg.rho_p,p,disc.T);
            FG=w./wmf;


            FGdiff=FG(end,:)-FG(1,:);
            FGdiff=reshape(FGdiff,sz);
        end
    end
