...
classdef Orifice
    methods(Static)
        %Mass flow through orifice plate
        ...
        %Persistent pressure loss across an orifice plate
        function [deltaOmega,p2]=deltaOmega(qm,p1,T1,d,D)
            persistent tap
            if isempty(tap)
                tap='D-D/2';
            end

            sz=implExp.size(qm,p1,T1,d,D);
            [qm,p1,T1,d,D]=implExp.normalize(sz,qm,p1,T1,d,D);
        
            p2=NaN(1,length(qm));
            for i=1:length(p2)
                check=Orifice.qm(p1(i),0,T1(i),d(i),D(i),tap)-qm(i);
                if check<0
                    p2Vec=linspace(0,p1(i),1000);
                    [~,idx]=max(Orifice.qm(p1(i),p2Vec,T1(i),d(i),D(i),tap)-qm(i));
                    p2(i)=p2Vec(idx);
                else
                    p2(i)=fzero(@(p2) Orifice.qm(p1(i),p2,T1(i),d(i),D(i),tap)-qm(i),[0,p1(i)]);
                end
            end
        
            [~,deltaOmega]=Orifice.qm(p1,p2,T1,d,D,tap);

            deltaOmega=reshape(deltaOmega,sz);
            p2=reshape(p2,sz);
        end
